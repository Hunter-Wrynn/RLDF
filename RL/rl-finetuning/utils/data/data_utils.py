...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "age_bias" in dataset_name:
        # chat_path = "/home/student2020/srcao/LLaMA-Factory-main/data/"
        # train_path = chat_path + "instruct_shuffle_age_bias_rephase_question_train_7064.json"
        # eval_path = chat_path + "instruct_shuffle_age_bias_rephase_question_test_v2_296.json"
        train_path = "./dataset/instruct_shuffle_age_bias_rephase_question_train_7064.json"
        eval_path = "./dataset/instruct_shuffle_age_bias_rephase_question_test_v2_296.json"
        if not (
            os.path.isfile(train_path)
            and os.path.isfile(eval_path)
        ):
            raise RuntimeError(
                f"Please check both the train.json and eval.json files in your applications/DeepSpeed-Chat/data directory."
            )
        print("train_data file exist.-----------")
        return raw_datasets.LocalAgeBiasJsonFileDataset(
            output_path, seed, local_rank, dataset_name, train_path, eval_path
        )
    elif "multi_role" in dataset_name:
        train_path = "./dataset/multi-role-sft-train.json"
        eval_path = "./dataset/multi-role-sft-test.json"
        if not (
            os.path.isfile(train_path)
            and os.path.isfile(eval_path)
        ):
